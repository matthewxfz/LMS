...
def main():
    ...
    table1 = open('Product.csv')
    table2 = open('Soldvia.csv')
...
<<<<<<< Updated upstream:HW/yann/database_manipulation.py
...
=======
    list_out_join = outjoin(reader1, reader2)
    print list_out_join
    with open('names.csv', 'w') as csvfile:
        rader1.fieldnames
        writer = csv.DictWriter(csvfile, fieldnames=w_fieldnames)
        writer.writeheader()
        for tuple_ in list_out_join:
            dictionary = dict(zip(w_fieldnames, tuple_))
   ...
   # for i in range(len(list_outjoin)):
    # print list_outjoin[i]
    # print len(outjoin(reader1, reader2))
>>>>>>> Stashed changes:HW2_yann/database_manipulation.py


def outjoin(reader1, reader2):
    for row1 in reader1:
        for row2 in reader2:
            w_fieldnames = row1.keys() + row2.keys()
            print row1
            print row2
    equal_column = []
    tuplelist = []

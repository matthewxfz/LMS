...
public class RecommendDAO extends BaseHibernateDAO {
	...
	public Recommend findById(edu.iit.dao.RecommendId id) {
		log.debug("getting Recommend instance with id: " + id);
		try {
			Recommend instance = (Recommend) getSession().get("edu.iit.dao.Recommend", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Recommend instance) {
		log.debug("finding Recommend instance by example");
		try {
			List results = getSession().createCriteria("edu.iit.dao.Recommend").add(Example.create(instance)).list();

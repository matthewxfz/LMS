...
/**
 	* A data access object (DAO) providing persistence and search support for Parents entities.
 			* Transaction control of the save(), update() and delete() operations 
		can directly support Spring container-managed transactions or they can be augmented	to handle user-managed Spring transactions. 
		Each of these methods provides additional information for how to configure it for the desired type of transaction control. 	
	 * @see edu.iit.dao.Parents
...
public class ParentsDAO extends BaseHibernateDAO  {
    ...
    public Parents findById( edu.iit.dao.ParentsId id) {
        log.debug("getting Parents instance with id: " + id);
        try {
            Parents instance = (Parents) getSession()
                    .get("edu.iit.dao.Parents", id);
            return instance;
        } catch (RuntimeException re) {
    ...
    public List findByExample(Parents instance) {
        log.debug("finding Parents instance by example");
        try {
            List results = getSession()
                    .createCriteria("edu.iit.dao.Parents")
    ...
    public List findByProperty(String propertyName, Object value) {
      log.debug("finding Parents instance with property: " + propertyName
            + ", value: " + value);
      try {
         String queryString = "from Parents as model where model." 
         						+ propertyName + "= ?";
    ...
    public Parents merge(Parents detachedInstance) {
        ...
        try {
            Parents result = (Parents) getSession()

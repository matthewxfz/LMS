...
/**
 	* A data access object (DAO) providing persistence and search support for RegisterTo entities.
...
public class RegisterToDAO extends BaseHibernateDAO  {
    ...
    public RegisterTo findById( edu.iit.dao.RegisterToId id) {
        ...
        try {
            RegisterTo instance = (RegisterTo) getSession()
                    .get("edu.iit.dao.RegisterTo", id);
            return instance;
        } catch (RuntimeException re) {
    ...
    public List findByExample(RegisterTo instance) {
        ...
        try {
            List results = getSession()
                    .createCriteria("edu.iit.dao.RegisterTo")
    ...
    public List findByProperty(String propertyName, Object value) {
      log.debug("finding RegisterTo instance with property: " + propertyName
            + ", value: " + value);
      try {
         String queryString = "from RegisterTo as model where model." 
         						+ propertyName + "= ?";
    ...
    public RegisterTo merge(RegisterTo detachedInstance) {
        ...
        try {
            RegisterTo result = (RegisterTo) getSession()

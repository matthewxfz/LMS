...
public class Queries {
	...
	/**
	 * SELECT <Attri>, COUNT(*) FROM
	 * <Table>
	 * GROUPBY <Attri> Having COUNT(*) > 1; SELECT City, COUNT(*) FROM Customers
	 * GROUP BY City HAVING COUNT(*) >1; Using bucket get projection and convert
	 * result, O(n)
	 * 
	 * @param file
	 */
	...
	/**
	 * SELECT <attri_2>, <attri_1> FROM Customers WHERE <attri_2> (SELECT
	 * <attri_2> FROM Orders WHERE <attri_3 >) FROM <Table1); SELECT CustomerID,
	 * CustomerName FROM Customers WHERE CustomerID IN (SELECT CustomerID FROM
	 * Orders WHERE EmployeeID = 9) ; O(n)
	 * 
	 * @param table
	...
	public List<String> linearNatureJoin(List<HashMap<String, String>> table1, List<HashMap<String, String>> table2) {
		...
		// Generate Two hashTable hashing from value from common key and rest
		// value
		HashMap<String, Tuple> hashTable1 = getColumneHashingTable(table1, comSchema, table1Schema);
		HashMap<String, Tuple> hashTable2 = getColumneHashingTable(table1, comSchema, table1Schema);
		...
		while (iks.hasNext()) {
			...
			if (hashTable2.containsKey(comKey)) {
				Tuple tuple1 = hashTable1.get(comKey);
				Tuple tuple2 = hashTable2.get(comKey);

				finalTuple = comKey;
				for (int i = 0; i < tuple1.count; i++) {
					finalTuple += "," + tuple1.values[i];
				}
				for (int i = 0; i < tuple2.count; i++) {
					finalTuple += "," + tuple2.values[i];
				}
	...
	/**
	 * Generate the HashMap whose key is the common key combination and value is
	 * the other values
	 * 
	 * @param table1
	 * @param comSchema
	 ...
	 * @return hash table
	 */
	private HashMap<String, Tuple> getColumneHashingTable(List<HashMap<String, String>> table, List<String> comSchema,
			List<String> disSchema) {
		HashMap<String, Tuple> hashTable = new HashMap<String, Tuple>();

		Iterator<HashMap<String, String>> tableIte = table.iterator();

		while (tableIte.hasNext()) {
			HashMap<String, String> HashTuple = tableIte.next();
			// add com part value
			Iterator<String> comIte = comSchema.iterator();
			String comKey = "";
			while (comIte.hasNext()) {
				comKey += HashTuple.get(comIte.next()) + ",";
			}
			// add distinct part value
			Iterator<String> disIte = disSchema.iterator();
			String disKey = "";
			while (disIte.hasNext()) {
				disKey += HashTuple.get(disIte.next()) + ",";
			}

			Tuple tuple = new Tuple(10);
			if (hashTable.containsKey(comKey)) {
				tuple = hashTable.get(comKey);
			}
			tuple.add(disKey);
			hashTable.put(comKey, tuple);
	...
	public List<String> natureJoin(List<HashMap<String, String>> table1, List<HashMap<String, String>> table2) {
		...
		// HashMap<String, String> emptyTuple1 =
		// getEmptyTuple(table1.iterator().next());
		// HashMap<String, String> emptyTuple2 =
		// getEmptyTuple(table2.iterator().next());
		// joint
	...
	class Tuple {
		...
		private String[] expanedArray(String[] array) {

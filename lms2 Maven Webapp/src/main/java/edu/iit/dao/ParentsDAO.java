...
public class ParentsDAO extends BaseHibernateDAO {
	...
	public Parents findById(edu.iit.dao.ParentsId id) {
		...
		try {
			Parents instance = (Parents) getSession().get("edu.iit.dao.Parents", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Parents instance) {
		...
		try {
			List results = getSession().createCriteria("edu.iit.dao.Parents").add(Example.create(instance)).list();

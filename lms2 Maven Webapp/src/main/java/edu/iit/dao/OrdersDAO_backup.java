 ...
 */
public class OrdersDAO_backup extends BaseHibernateDAO {
	private static final Logger log = LoggerFactory.getLogger(OrdersDAO_backup.class);
	// property constants
	...
	public List findALLBooks_book_ByStudentId(int pageNumber, int pageSize, int studentId) {
		try {
			...
			for (int i = 0; i < overdue_order.size(); i++) {
				overdue_book.add(bdao.findById(overdue_order.get(i).getBookId()));
			}
			return overdue_book;
		} catch (RuntimeException re) {
			log.error("find by property name failed", re);
...
//			List<Books> overdue_book = new ArrayList<Books>();
//			for (int i = 0; i < overdue_order.size(); i++) {
//				overdue_book.add(overdue_order.get(i));
//			}
//			return overdue_book;
//		} catch (RuntimeException re) {
//			log.error("find by property name failed", re);
...
//	}
	...
	/**
	 * Return the open order for bookid and studentId, if no order is still open, return null.
	 * @param bookId
	 * @param studentId
	 * @return null/Order
	 */
	public Orders findOpenOrderByBookIDAndStudentID(int bookId, int studentId) {
		try {

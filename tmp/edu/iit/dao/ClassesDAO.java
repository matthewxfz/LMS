...
public class ClassesDAO extends BaseHibernateDAO {
	...
	public List groupByProperty(String propertyName, Object value) {
		log.debug("finding Classes instance with property: " + propertyName + ", value: " + value);
		try {
			String queryString = "SELECT COUNT(c.classId) AS total FROM Classes AS c GROUP BY c.section";
			Query queryObject = getSession().createQuery(queryString);
			//queryObject.setParameter(0, value);
			return queryObject.list();
		} catch (RuntimeException re) {
	...
	public List countWhere(String propertyName, Object value) {
		log.debug("finding Classes instance with property: " + propertyName + ", value: " + value);
		try {
			String queryString = "select count(ClassID) from Classes where Section = 1";
			Query queryObject = getSession().createQuery(queryString);
			//queryObject.setParameter(0, value);
			return queryObject.list();
		} catch (RuntimeException re) {
